---
name: ui-ux-pro-max
description: "Professional UI/UX design workflows, component systems, and design tokens."
version: 1.0.0
author: Community
license: MIT
platforms: [linux, macos, windows]
metadata:
  hermes:
    tags: [marketplace, community]
---

# UI/UX Pro Max

Professional UI/UX design workflows, component systems, and design tokens.

## Installation

```bash
npx mdskills install nextlevelbuilder/ui-ux-pro-max
```

## Rating

9/10

## Source

GitHub: [nextlevelbuilder/ui-ux-pro-max](https://github.com/nextlevelbuilder/ui-ux-pro-max)

---

## How to use?

### 1. Download
Go to the [AGENT KD SKILL gallery](https://ctaxnagomi.github.io/agent-kd-skill/) and find this skill card. Click **Download** to save the SKILL.md file.

### 2. Install
Copy the downloaded SKILL.md into your agent's skills directory:

| Agent | Path |
|-------|------|
| **opencode** | ~/.config/opencode/skills/ui-ux-pro-max/SKILL.md |
| **Claude Code** | ~/.claude/skills/ui-ux-pro-max/SKILL.md |
| **Hermes** | ~/.hermes/skills/marketplace/ui-ux-pro-max/SKILL.md |

### 3. Use
Restart your agent session. The skill will auto-load based on its description.

### 4. Verify
Ask your agent something related to the skill's purpose. If it responds using the skill's instructions, you're all set.
