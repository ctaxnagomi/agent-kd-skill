---
name: cursor-aspnet-abp
description: "ASP.NET ABP framework for Cursor AI."
version: 1.0.0
author: Community
license: MIT
platforms: [linux, macos, windows]
metadata:
  hermes:
    tags: [marketplace, community]
---

# ASP.NET ABP

ASP.NET ABP framework for Cursor AI.

## Installation

```bash
npx mdskills install PatrickJS/cursor-aspnet-abp
```

## Rating

7/10

## Source

GitHub: [PatrickJS/cursor-aspnet-abp](https://github.com/PatrickJS/cursor-aspnet-abp)

---

## How to use?

### 1. Download
Go to the [AGENT KD SKILL gallery](https://ctaxnagomi.github.io/agent-kd-skill/) and find this skill card. Click **Download** to save the SKILL.md file.

### 2. Install
Copy the downloaded SKILL.md into your agent's skills directory:

| Agent | Path |
|-------|------|
| **opencode** | ~/.config/opencode/skills/cursor-aspnet-abp/SKILL.md |
| **Claude Code** | ~/.claude/skills/cursor-aspnet-abp/SKILL.md |
| **Hermes** | ~/.hermes/skills/marketplace/cursor-aspnet-abp/SKILL.md |

### 3. Use
Restart your agent session. The skill will auto-load based on its description.

### 4. Verify
Ask your agent something related to the skill's purpose. If it responds using the skill's instructions, you're all set.
